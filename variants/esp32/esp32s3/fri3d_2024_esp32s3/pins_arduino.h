...
// pfodWeb NOTE: SS/MOSI/MISO/SCK not declared — this is the dedicated
// SPI bus for the onboard ST7789 TFT (TFT_CS=5/DC=4/RST=48 below) and
// SD card (PIN_SDCARD_CS=SS), not a general-purpose SPI header.
...
// pfodWeb NOTE: A<n>/T<n> below deliberately omit entries for GPIO4-8
// (TFT/SD SPI bus), 10/11 (IR blaster/receiver), 14 (SPI SS), 15/17
// (addon mic), and 19/20 (native USB D-/D+) — every static const alias
// here (regardless of A/T name) adds its GPIO to the pin list, so simply
// not declaring these avoids re-leaking pins already excluded above.
