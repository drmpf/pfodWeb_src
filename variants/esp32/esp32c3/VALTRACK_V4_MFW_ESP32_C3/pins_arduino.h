...
// boardsDetails/esp32/esp32c3/VALTRACK_V4_MFW_ESP32_C3/notes.txt for the
...
// confirmed as a real SPI bus on this board (the firmware source review
// found no SPI peripheral at all), and they silently contaminated the
// confirmed GPIO_SOS button and GPIO_LED_SIGNAL NeoPixel's capability
// lists with "spi_miso"/"spi_sck" (build_boards.js appends these
// unconditionally to any GPIO matching the MISO/SCK alias, the same
// mechanism as the TX/RX hard-reservation bug). SS (GPIO20) left as-is -
// it coincides with this board's RX pin, which is handled specially by
// build_boards.js before the SPI-alias injection runs, so it's harmless.
