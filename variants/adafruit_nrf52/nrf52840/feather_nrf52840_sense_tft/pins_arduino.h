...
// Adafruit_nrf52/variants/feather_nrf52840_sense_tft/variant.h, filename
...
// sentinel meaning "this analog channel doesn't physically exist on
// this board" — but build_boards.js's alias-scanning regex only
// understands DECIMAL numeric literals, not hex: it misreads "0xff" as
// decimal "0", so the alias would have silently resolved to a phantom
// "A7 (GPIO0)" pin. Deleting the alias line means this board correctly
// has no A7 pin at all (identical fix to itsybitsy_nrf52840_express and
// the pca10056/pca10100 DKs in this family — see this board's own
// boardsDetails/adafruit_nrf52/nrf52840/feather_nrf52840_sense_tft/
// notes.txt).
//
// ALSO NOTE: this vendor file's own header include-guard is literally
// "_VARIANT_CLUE52840_" (copy-pasted from Adafruit's CLUE board file,
// a genuinely different board with a genuinely different pin layout) —
// a cosmetic vendor bug with no functional effect (this repo never
// compiles the file, only regex-scans it, and C include guards don't
// leak between separately-processed files anyway), preserved verbatim
// below for fidelity to the vendor source.
 /*
