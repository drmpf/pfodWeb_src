...
// (see board.json chipGpios), and the onboard MCP25625 CAN transceiver
// uses its own dedicated PIN_CAN_CS (GPIO19) rather than this default SS
// assignment, so leaving it declared would wrongly tag the confirmed LED
// pin with a spurious spi_ss capability (see build_boards.js's
// unconditional SS-alias auto-append). MISO/MOSI/SCK are left as-is -
// per Adafruit's own pinouts guide these are "the main exposed SPI"
// header pins, shared with (not dedicated to) the onboard CAN
// transceiver, and remain available for other SPI devices.
