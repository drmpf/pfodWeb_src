...
// pfodWeb NOTE: this is the 48-pin RP2350B variant of ../generic/common.h,
// for the one board in this variant set that is genuinely RP2350B-based
// (jumperless_v5, which declares `#define PICO_RP2350A 0 // RP2350B`
// itself) but still relies on the shared common.h fallback for its D<n>/
// A<n> numbering. The upstream arduino-pico common.h picks between the
// 30-pin (RP2040/RP2350A) and 48-pin (RP2350B) pin layouts via #if/#elif
// on PICO_RP2350A — since build_boards.js's pin parser is a plain regex
// scan with no #if evaluation, the two branches can't coexist in one
// file (see ../generic/common.h for the full rationale on why that file
// only keeps the 30-pin branch). This file is the 48-pin-only
// counterpart, kept in a separate directory precisely so it never gets
// included by any 30-pin board.
