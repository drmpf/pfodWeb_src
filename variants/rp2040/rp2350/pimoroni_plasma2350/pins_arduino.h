...
// pfodWeb NOTE: a dead `#if 0 ... #endif`-disabled "Digital pins" block
// (D0-D10 static const uint8_t declarations) was removed entirely from
// here — this project's .h parser is a plain regex text scan with no
// preprocessor awareness, so it still matched those `static const
// uint8_t` lines despite the `#if 0` and leaked phantom D0-D10 pins
// (including a re-leak of GPIO29, already excluded above as A3/"not
// pinned out"). Deleted rather than left disabled/commented, matching
// this file's existing A3 and SPI SS fixes (same bug class).
