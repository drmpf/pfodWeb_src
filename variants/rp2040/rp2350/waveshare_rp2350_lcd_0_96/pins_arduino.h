...
// PIN_WIRE0_SDA/SCL removed — this board's vendor-default Wire0 (I2C0)
// location (GPIO8/GPIO9) is physically the same pair of pins dedicated to
// the onboard LCD's DC and CS lines (see PIN_LCD_DC/PIN_LCD_CS above), so
// Wire0 can never actually be used at its declared default location on
// this board while the display is present. Confirmed identical to the
// waveshare_rp2040_lcd_0_96 sibling's already-fixed collision (see that
// board's board.json/notes.txt), and independently confirmed present in
// the real earlephilhower/arduino-pico vendor source for this exact
// RP2350 variant (not just this repo's mirror). Left in place, the
// `static const uint8_t SDA = PIN_WIRE0_SDA;` / `SCL = PIN_WIRE0_SCL;`
// aliases below would cause build_boards.js to unconditionally tag GPIO8/9
// as i2c_sda/i2c_scl (its bus-role auto-append keys off the SDA/SCL alias
