 ...
 * in the shared generic/common.h the way the stock rpipicow variant does)
 * because Pico W wires GPIO23/24/25/29 internally to the onboard CYW43
 * WiFi/BLE chip's SPI-ish control interface — they are NOT available as
 * general-purpose GPIOs on this board, even though the RP2040 silicon has
 ...
 * offered with a caveat. Confirmed against the Pico W schematic and the
 * arduino-pico core's cyw43_wrappers.h.
 */

// LEDs — LED_BUILTIN lives on the WiFi chip (cyw43_wrappers.h intercepts
// pin 64 specially), not a real RP2040 GPIO; not parsed as a pin here.
#define PIN_LED        (64u)
