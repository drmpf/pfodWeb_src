...
// Seeed_nRF52/variants/Seeed_XIAO_nRF52840_Plus/variant.h
// (Adafruit_nRF52_Arduino-derived core), filename changed to
// pins_arduino.h per this repo's convention, with ONE deliberate
// deviation from the vendor source: the vendor file's two generic
// bare-named pin-alias declarations for the hardware UART (which would
// alias to the same GPIOs as PIN_SERIAL1_RX/PIN_SERIAL1_TX below) have
// been deleted outright (not commented out — a commented-out
// declaration still matches this repo's alias-scanning regex, since it
// has no C-comment awareness; see the pin-audit playbook's parser-
// gotchas section). Deleted, not merely disabled, for that reason.
//
...
// see boardsDetails/nrf52/nrf52840/Seeed_XIAO_nRF52840_Plus/notes.txt
