...
use windows::Devices::Bluetooth::Advertisement::{
    ...
    // BluetoothLEAdvertisementType removed with the pfodProxy local patch
    // below — it had no other use.  See ../../VENDOR-PATCH.md.
    ...
    pub async fn scan<'a>(
        ...
        Ok(receiver
            .then(move |event_args| {
                ...
                Box::pin(async move {
                    // ┌── pfodProxy LOCAL PATCH ── see ../../VENDOR-PATCH.md ──┐
                    // Upstream returns None here when
                    //   event_args.AdvertisementType() == NonConnectableUndirected
                    // on the grounds that "Device cannot be created from a
                    // non-connectable advertisement".  Removed: that is a
                    // property of the advertisement Windows *reports*, not of
                    // the device, and this Bluetooth stack mis-reports it.
                    // Measured over 20 s, an Arduino advertising the Nordic
                    // UART Service with Flags 0x06 (LE General Discoverable,
                    // BR/EDR not supported) had all 62 of its ADV_IND packets
                    // typed NonConnectableUndirected — while Device::from_addr
                    // on the very same address opened it and ran a full GATT
                    // session.  The same stack also reports +27 dBm RSSI, which
                    // no BLE radio emits, so its advertisement metadata is not
                    // trustworthy in general.
                    //
                    // Nothing is lost by dropping the check: the from_addr call
                    // below already returns None for a device that genuinely
                    // cannot be opened, which is the case this was guarding.
                    // └────────────────────────────────────────────────────────┘
