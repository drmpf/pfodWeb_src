...
function buildBoardHierarchy(boardDataById) {
  ...
  // Sort boards within each chip alphabetically by display name, except
  // a "Generic ..." entry (the vendor's own catch-all/default board,
  // e.g. "Generic ESP8266 Module") always sorts first — it's the
  // sensible default pick and shouldn't be buried wherever "G" happens
  // to fall alphabetically among a chip's other board names.
  const _isGeneric = (name) => /^generic\b/i.test(name);
  for (const fam of Object.values(hierarchy)) {
    for (const chip of Object.values(fam.chips)) {
      chip.boards.sort((a, b) => {
        const aGeneric = _isGeneric(a.name);
        const bGeneric = _isGeneric(b.name);
  ...
  bundles: [
    {
      ...
      scripts: [
        ...
        // INO_TEMPLATES — auto-generated by build_boards.js from every
        // distinct variants/**/*.ino file.  Per-board connections[proto]
        // entries below reference into it by inoTemplateId; must load
        // before generateCode.js, which looks the id up at code-gen time.
