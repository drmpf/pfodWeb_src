...
const DesignerGenerateCode = (() => {
  ...
  // ── .h generator ──────────────────────────────────────────────────
  // pfodMainMenu is a real class (virtual sendMainMenu/sendMainMenuUpdate
  ...
  function _generateH(state) {
    ...
    out += '  public:\n';
    out += '    pfodMainMenu();\n';
    out += '    void init(pfodCloseConnectionPtr closeConnectionFn = NULL);\n';
  ...
  function _generateCpp(state) {
    ...
    // pfodAutoCmd variable declarations — one per item (main menu's own
    // items only — a sub-menu's own items are declared in its own
    // SubMenu_<ident>.cpp instead, see _generateSubMenuFiles).
    for (const item of items) {
      const label = _cppStr((item.text || '').replace(/\n/g, ' ').trim() || item.type);
    ...
    out += 'static pfodMainMenu mainMenu;\n\n';
    ...
    // to be inlined directly in the mechanical dispatch branch above.
    for (const item of items) {
    ...
    out += 'handle_mainMenuFnPtr init_pfodMainMenu(pfodCloseConnectionPtr closeConnectionFnPtr) {\n';
    out += '  mainMenu.init(closeConnectionFnPtr);\n';
  ...
  function _generateSubMenuFiles(item, names, state) {
    ...
    if (drawings.length > 0) {
      cpp += '#include <pfodDwgs.h>\n';
      cpp += '#include <pfodDrawing.h>\n';
    ...
    // pfodAutoCmd / int variable declarations for this level's own items.
    for (const sItem of items) {
      const sLabel = _cppStr((sItem.text || '').replace(/\n/g, ' ').trim() || sItem.type);
      if (sItem.type === 'onoff') {
        cpp += 'int ' + _intVarName(sItem.autoCmd) + ' = ' + sItem.current + '; // \'' + sLabel + '\'\n';
      } else if (sItem.type === 'pwm') {
        cpp += 'int ' + _intVarName(sItem.autoCmd) + ' = ' + sItem.minValue + '; // \'' + sLabel + '\'\n';
      }
      if (sItem.type === 'drawing') {
        cpp += 'pfodAutoCmd ' + _dwgCmdVarName(sItem.autoCmd) + '; // drawing\n';
      } else {
        cpp += 'pfodAutoCmd ' + _cmdVarName(sItem.autoCmd) + '; // ' + sItem.type + ' -- \'' + sLabel + '\'\n';
      }
    }
    cpp += '\n';
    ...
    // tickCharts -- own charts + delegate to every nested child.
  ...
  function _triggerDownload(state) {
    ...
    const entries = [
      ...
      { path: name + '/menuJson/' + name + '.pfodMenu_json', data: enc.encode(state.exportToJSON()) },
    ];
    dwgFiles.forEach((f) => entries.push({ path: name + '/' + f.filename, data: enc.encode(f.content) }));
    ...
    dwgNames.forEach((dwgName) => {
      const dwg = DwgLibrary.get(dwgName);
      entries.push({
        path: name + '/menuJson/' + dwgName + '.pfodDwg_json',
    ...
    topLevelSubmenus.forEach((item) => {
      ...
      entries.push({ path: name + '/' + gen.className + '.h',   data: enc.encode(gen.h) });
      entries.push({ path: name + '/' + gen.className + '.cpp', data: enc.encode(gen.cpp) });
      gen.childFiles.forEach((f) => entries.push({ path: name + '/' + f.filename, data: enc.encode(f.content) }));
    });
