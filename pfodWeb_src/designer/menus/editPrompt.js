...
const DesignerEditPrompt = (() => {
  ...
  function send(rawCmd, state, depth) {
    ...
    // Bare `{n}`/terminator, OR editMenu.js's own level-suffixed
    // bare-open (designerLevelSuffix, formats.js — digits/underscores
    // then a fixed 'x', with nothing meaningful after) — both mean
    // "show the full-form parent screen". Sub-command clicks below
    // (s/b/i/u/f/o/w/c/B) are generated by THIS screen's own
    // rendering, not editMenu.js's button, so they never carry the
    // suffix and dispatch exactly as before.  skipSave because the
    // render itself doesn't change state.
    if (sub === undefined || sub === '}' || designerIsLevelSuffixOnly(rawCmd, depth + 1)) {
