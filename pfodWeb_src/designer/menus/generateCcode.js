...
const DesignerGenerateCcode = (() => {
  ...
  /// so every generator function below (pfodMenu.h/.c, main.c,
  /// pfodParserStream.c) stays in sync. Not applied to pfodParser.h/.c/
  /// pfodParserStream.h — those are the real pfodParserC library's own
  /// source, inlined verbatim, and already carry that library's own
  /// copyright as shipped.
  ...
  function _triggerCcodeDownload(state) {
    ...
    // Filesystem-safe form of the design name for every folder/file/zip
    // name below — state.name itself is free text and stays as-is
    // everywhere else (e.g. inside the generated files' own comments/
    // JSON `name` field). Matches generateCode.js's own _triggerDownload.
    const fileName = _cId(name) || 'Menu';
    const entries = [
      { path: fileName + '/pfodMenu.h',         data: enc.encode(_generatePfodMenuH(state, items, charts, pulseItems, bufSize)) },
      { path: fileName + '/pfodMenu.c',         data: enc.encode(_generatePfodMenuC(state, tree, items, charts, pulseItems)) },
      { path: fileName + '/pfodParser.h',       data: enc.encode(PFOD_PARSER_H_TEXT) },
      { path: fileName + '/pfodParser.c',       data: enc.encode(PFOD_PARSER_C_TEXT) },
