 ...
 * — so the design can be re-opened later via Load Dwg).
 *
...
const DwgArduinoExport = (() => {
  ...
  /// plain. Each touchZone gets its own public, virtual dummy per-cmd
  /// method declared here (Dwg_<name>_<cmdName>(row, col, touchType,
  /// editedText)) for the user to fill in directly in the .cpp, or
  /// override from their own subclass.
  ...
  function _generateHeader(dwg) {
    ...
    (dwg.items || []).forEach((item) => {
      ...
      code += '    virtual void Dwg_' + name + '_' + item.cmdName +
        '(int row, int col, uint8_t touchType, const byte* editedText); // touchZone ' + item.cmdName + ' touched\n';
    });
  ...
  function _generateCpp(dwg, missingDwgSet) {
    ...
    items.forEach((item) => {
      ...
      code += '  if (parserPtr->dwgCmdEquals(' + item.cmdName + ')) { // handle touchZone ' + item.cmdName + '\n' +
        '    Dwg_' + name + '_' + item.cmdName + '(parserPtr->getTouchedRow(), parserPtr->getTouchedCol(), parserPtr->getTouchType(), parserPtr->getEditedText());\n' +
        '    sendUpdate(); // or sendUpdate from here if only this dwg needs updating\n' +
        '    return true;\n' +
        '  }\n';
    });
    ...
    seenCmds.forEach((cmdName) => {
      code += 'void Dwg_' + name + '::Dwg_' + name + '_' + cmdName +
