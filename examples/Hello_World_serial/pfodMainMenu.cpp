 ...
 */
/* ===== pfod Command for Hello_World_serial ====
...
void pfodMainMenu::init(pfodCloseConnectionPtr _closeConnectionFnPtr) {
  ...
  get_dwg_HelloWorld().init(); // initialize drawing -- 'Hello World'
}
...
void pfodMainMenu::handle(pfodParser& parser) {
  ...
  if (cmd != 0) { // have parsed a complete msg { to }
    ...
    } else if(parser.cmdEquals(dwgMenuItem_Drawing_Cmd)) { // user touch not handled by dwg, handle it here
      // drawing loadCmd handled internally by get_dwg_xxx().init()
...
void pfodMainMenu::sendMainMenu(pfodParser& parser) {
  ...
  parser.print(dwgMenuItem_Drawing_Cmd); // drawing menu item cmd
  parser.print(F("~"));
  parser.print(get_dwg_HelloWorld()); // the drawing's loadCmd
...
void pfodMainMenu::sendMainMenuUpdate(pfodParser& parser) {
  ...
  parser.print(dwgMenuItem_Drawing_Cmd);
  parser.endOfMsg();  // close pfod message. Send }
  // ============ end of menu ===========
}
