...
/**
 ...
 The project has been tested using Arduino IDE V2.3.6, Pi Pico board support V5.1.0,  ESP32 board support V3.3.1, ESP8266 board support V3.1.2
  ...
  Set the ssid and password (see WiFi Settings in the code below) to match your local network's router, ssid and password
  Set a static IP to an unused IP on your network OR leave as blank and check the Serial monitor for the assigned IP
...
const char staticIP[] = "10.1.1.100";  // set this the static IP you want, e.g. "10.1.1.200" or leave it as "" for DHCP. DHCP is not recommended.
...
#endif
  IPAddress ip;
  bool validStaticIP = ip.fromString(staticIP);
...
void loop() {
  ...
  if (server.hasClient()) {         // new connection
    if (!clientPtr) {               // no existing connection
      client = server.available();  // get any new client
      clientPtr = &client;
// have connected client
#ifdef DEBUG
      Serial.println("new connection");
#endif
      parser.connect(clientPtr);  // sets new io stream to read from and write to
    } else {                      // already have client so just stop the new one
#ifdef DEBUG
      Serial.println("Already have connection stop this one");
#endif
      WiFiClient newClient = server.available();  // get any new client
      newClient.stop();
    }
  }
...
void closeConnection(Stream* /*io*/) {
  ...
  rawDataDelay.stop();  // no connection
  rawDataArmed = false;
#ifdef DEBUG
