 ...
 */
/* ===== pfod Command for LedOnOff_serial ====
...
void pfodMainMenu::init(pfodCloseConnectionPtr _closeConnectionFnPtr) {
  ...
  get_dwg_LedOnOff().init(); // initialize drawing -- 'LedOnOff'
}
...
void pfodMainMenu::handle(pfodParser& parser) {
  ...
  if (cmd != 0) { // have parsed a complete msg { to }
    ...
    } else if(parser.cmdEquals(dwgMenuItem_LedOnOff_Cmd)) { // user touch not handled by dwg, handle it here
...
void pfodMainMenu::sendMainMenu(pfodParser& parser) {
  ...
  parser.print(dwgMenuItem_LedOnOff_Cmd); // drawing menu item cmd
  parser.print(F("~"));
  parser.print(get_dwg_LedOnOff()); // the drawing's loadCmd
...
void pfodMainMenu::sendMainMenuUpdate(pfodParser& parser) {
  ...
  parser.print(dwgMenuItem_LedOnOff_Cmd);
  parser.endOfMsg();  // close pfod message. Send }
  // ============ end of menu ===========
}
